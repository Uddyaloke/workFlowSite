...
class workListAdmin(admin.ModelAdmin):
	...
	def items_pending_my_action(self, obj):
		...
		if chk_user or self.user.is_superuser:
			if model_name == empMaster:
				items = model_name.objects.filter(status_id=6)

				for p in items:
					wl_items += format_html("<li><a href='{loc}'>{item_name}</a></li>",item_name=str(p.emp_id) + " - " + p.emp_name, loc="/admin/empMaster/empmaster/"+str(p.id)+"/change")
